...
import { Mail, Lock, User, Phone, ShieldCheck, Eye, EyeOff, Store, Loader2 } from 'lucide-react';
...
const schema = z.object({
  ...
  phone: z.string().min(7, 'Enter a valid phone number').max(16).regex(/^\+?[1-9]\d{5,14}$/, 'Enter a valid phone number (e.g. +254 700 000000)'),
...
function RegisterForm() {
  ...
  return (
    <div className="bg-white dark:bg-maasai-brown rounded-3xl shadow-2xl border border-maasai-beige/30 dark:border-maasai-brown-light p-8">
      {/* Google */}
      <button type="button" onClick={signUpWithGoogle} disabled={googleLoading}
      ...
      <p className="text-center text-xs text-maasai-brown/50 dark:text-maasai-beige/50 mt-2">
        Signs you up as a Buyer — upgrade to Seller in your profile
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <label key={value} className={cn(
                ...
                selectedRole === value ? 'border-maasai-red bg-maasai-red/5' : 'border-maasai-beige dark:border-maasai-brown-light hover:border-maasai-red/50'
              )}>
        ...
        {selectedRole === 'seller' && (
          <div>
            <label className="block text-sm font-semibold text-maasai-brown dark:text-maasai-beige mb-2">
              Shop / Brand Name <span className="text-xs font-normal text-maasai-brown/60">(optional)</span>
            </label>
            <div className="relative">
              <Store className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-maasai-brown/50" />
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input {...register('phone')} placeholder="+254 700 000000" className={inputCls(!!errors.phone)} />
          </div>
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-maasai-brown/80 dark:text-maasai-beige/80">
            I agree to the{' '}
            <Link href="/terms" className="text-maasai-red hover:underline">Terms</Link> and{' '}
            <Link href="/privacy" className="text-maasai-red hover:underline">Privacy Policy</Link> (KDPA 2019)
          </span>
        </label>
      ...
      <p className="text-center text-sm text-maasai-brown/70 dark:text-maasai-beige/70 mt-6">
        ...
        <Link href="/login" className="text-maasai-red font-semibold hover:underline">Sign in</Link>
      </p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-maasai-cream dark:bg-maasai-black p-4 py-10">
      <div className="w-full max-w-lg">
        ...
        <Suspense fallback={
          <div className="bg-white dark:bg-maasai-brown rounded-3xl shadow-2xl border border-maasai-beige/30 p-8 flex items-center justify-center min-h-[400px]">
